...
- July 23, 2025. Enhanced car receipt form for reception workflow: added "بطاقة زبون" tab to reception interface for customer management access, modified car receipt form to make all customer and car data fields read-only (populated from customer database search), changed fuel level from dropdown to manual text input, removed license plate field duplication as users search for cars first then create receipts, enforced data integrity by preventing manual editing of customer/car information during receipt creation
