...
- July 23, 2025. Enhanced car receipt form for reception workflow: added "بطاقة زبون" tab to reception interface for customer management access, modified car receipt form to make all customer and car data fields read-only (populated from customer database search), changed fuel level from dropdown to manual text input, removed license plate field duplication as users search for cars first then create receipts, enforced data integrity by preventing manual editing of customer/car information during receipt creation, added customer creation permissions to reception account for complete workflow support, restricted "إرسال للورشة" button to only فارس user account, fixed car status management component access for بدوي user
- July 23, 2025. Implemented car postponement workflow: added "تأجيل" (postpone) button for "بدوي" user to postpone car entry to workshop, created "postponed" status with database fields (postponedAt, postponedBy), when car is postponed only "إدخال" (enter) button appears for workshop entry, fixed WebSocket message handling for WORKSHOP_NOTIFICATION and CAR_RECEIPT_CREATED types, enhanced car status display with proper color coding and status messages for postponed cars, updated postponed car status text to "بانتظار التسليم للورشة" for clearer workflow understanding
- July 23, 2025. Enhanced car status management with comprehensive workshop activity display: integrated parts requests and active tasks to show complete workshop activity for each car, displays current work details (task description, repair operation, task type, assigned workers), shows parts requests with status and quantities, provides detailed workshop activity visibility for reception department to track all ongoing work on each vehicle, simplified receipt numbering to sequential format (استلام-1, استلام-2, etc.)
- July 30, 2025. Fixed PostCSS configuration issue: converted postcss.config.js from CommonJS to ES module format to resolve startup failures with "type": "module" in package.json
- July 30, 2025. Implemented desktop application functionality using Electron: created main.js for desktop interface, added electron-simple.js for quick startup, created desktop launch scripts (start-desktop.bat, start-desktop.sh, start-desktop.js), generated custom SVG icon and configuration files, provided comprehensive desktop setup documentation, enables running the system as standalone desktop application without browser dependency
