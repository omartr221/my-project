  ...
  try {
    ...
    for (const table of tables) {
      ...
      for (const row of rows) {
        ...
        // إدراج في PostgreSQL
        const columns = Object.keys(row).filter(k => k !== 'id').join(', ');
        const values = Object.values(row).filter((_, i) => Object.keys(row)[i] !== 'id');
        ...
        try {
          ...
          console.log('البيانات:', row);
        }
      }
      
      console.log(`✅ تم نقل ${table}`);
    }
    ...
    for (const table of tables) {
      try {
        await pool.query(`SELECT setval(pg_get_serial_sequence('${table}', 'id'), (SELECT MAX(id) FROM ${table}) + 1);`);
      } catch (error) {
        console.log(`تحذير: لم يتم تحديث تسلسل ${table}`);
      }
    }
