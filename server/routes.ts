...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/backup/create", async (req, res) => {
    if (
      !req.isAuthenticated() ||
      !(req.user as any)?.permissions?.includes("admin")
    ) {
      return res
        .status(401)
        .json({ message: "غير مصرح لك بإنشاء نسخ احتياطية" });
    }
  ...
  app.post("/api/backup/restore", async (req, res) => {
    ...
    if (result.success) {
      ...
      res
        .status(500)
        .json({ message: "فشل في استعادة البيانات", error: result.error });
    }
  });
  ...
  app.patch("/api/tasks/:id", async (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "الوقت المقدر يجب أن يكون رقم موجب" });
      }
  ...
  app.post("/api/tasks/:id/cancel", async (req, res) => {
    try {
      ...
      if (!cancelledBy || !reason) {
        return res
          .status(400)
          .json({ message: "cancelledBy and reason are required" });
      }
  ...
  app.post("/api/parts-requests", async (req, res) => {
    try {
      ...
      if (request.engineerName && request.partName) {
        ...
        // تسجيل الحدث للتحقق
        console.log(
          "🔔 Broadcasting parts request notification:",
  ...
  app.patch("/api/parts-requests/:id/status", async (req, res) => {
    try {
      ...
      const request = await storage.updatePartsRequestStatus(
        id,
        status,
        notes,
        estimatedArrival,
      ...
      res
        .status(500)
        .json({ message: "Failed to update parts request status" });
    }
  });
  ...
  app.put("/api/parts-requests/:id/return", async (req, res) => {
    try {
      ...
      const request = await storage.returnPartsRequest(
        id,
  ...
  app.post("/api/car-receipts/:id/send-to-workshop", async (req, res, next) => {
    try {
      ...
      const receipt = await storage.sendCarReceiptToWorkshop(
        receiptId,
        (req.user as any)?.username || "مجهول",
      );
