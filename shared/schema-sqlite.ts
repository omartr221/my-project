...
export const partsRequests = sqliteTable("parts_requests", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  requestNumber: text("request_number", { length: 20 }).unique().notNull(),
  licensePlate: text("license_plate", { length: 20 }).notNull(),
  customerName: text("customer_name", { length: 100 }).notNull(),
  carBrand: text("car_brand", { length: 50 }).notNull(),
  carModel: text("car_model", { length: 100 }).notNull(),
  ...
  partDescription: text("part_description", { length: 500 }).notNull(),
  quantity: integer("quantity").notNull().default(1),
  priority: text("priority", { length: 20 }).notNull().default("normal"), // urgent, normal, low
  status: text("status", { length: 30 }).notNull().default("in_preparation"), // in_preparation, awaiting_pickup, parts_arrived, delivered
  requestedBy: text("requested_by", { length: 100 }).notNull(),
